...
function emptyTabElement(element_id) {
...
(function($) {
  ...
  $.fn.contextShow = function(options) {
    return this.each(function() {
      var element = $(this);
      var value;
      if ($(this).is(':checkbox')) {
      ...
      var selector = $(this).data('context');
      var to_show = selector + "-" + value;
      $(selector + ":not(" + to_show + ")").hide().find(':input').unrequire();
      $(to_show).show().find(':input').rerequire();
    });
  };

  $.fn.toggleClassForClass = function(options) {
    return this.each(function() {
      var element = $(this);
      element.toggleClass(options);
    });
  };

  $.fn.removeClassForClass = function(options) {
    return this.each(function() {
      var element = $(this);
  ...
  $.fn.toggleClassForClassParent = function(options) {
    return this.each(function() {
      var element = $(this);
      element.parent().toggleClass(options);
    });
  };
  ...
  $.fn.unrequire = function(options) {
    return this.each( function() {
      var that = $(this);
      that.data('was-required', that.prop('required'));
      that.prop('required', false);
    });
  };

  $.fn.rerequire = function(options) {
    return this.each( function() {
      var that = $(this);
      if (that.data('was-required')) {
        that.prop('required', that.data('was-required'));
      }
    });
  };
  ...
  $.fn.performScroll = function(windowBased) { 
    ...
    var next_page_url = target_element.data('next-page-url');
    if (next_page_url == undefined || next_page_url == '' ) return;
    ...
    if (windowBased) {
      ...
      var position = more.offset().top - this_top; // more data's top
  ...
  $.fn.newHTML = function(html, options) {
    var thisHtml = this.each( function() {
      $(this).html(html);
...
//      .find('[type="checkbox"].switch').bootstrapSwitch().on('switchChange.bootstrapSwitch', function(event, state) {
 //       if ($(this).data('toggle') ) $($(this).data('toggle')).toggle();
  ...
  $.fn.collectedSubmit = function(options) {
    ...
    /*
    var hideTarget = element.data('add-hide-to');
    if (hideTarget != undefined && hideTarget != '' ) {
      if ( $(this).is("select") ) {
        var val = $(this).find('option:selected').attr("value");
        $(hideTarget).alterClass('hide-*', "hide-" + val );
      }
    }
    */
    ...
    $(collect).each( function() {
      var thisIsFilter=$(this).hasClass('filter-active');
      if ( thisIsFilter ) { 
        // hidden field holding filter_details
        // {"filter_hash"=>{"ass"=>[""], "ed"=>"", "rr"=>[""], "sd"=>"", "sp"=>[""], "ts"=>["", "0", "2"], "tt"=>["", "3", "15"], "us"=>[""]}} //

        var filter_details = $('#filter_details').val();
        if (filter_details)
        {
          // Step 1 - replace => to :
          filter_details = filter_details.replace(/=>/g, ":"); 

          // Step 2 - Parse To JSON
          var filter_data = JSON.parse(filter_details);

          // Step 3 - Add Into Data For Ajax
          if ("filter_hash" in filter_data) {
            data['filter_details[filter_hash]'] = filter_data["filter_hash"];
          }
        }
      } 
        ...
        if (thisIsSort) {
          ...
          if (key) {
            ...
            if ($(this).data("value") === undefined ) {
              data[key].push( $(this).attr("id") );
            } else {
              data[key].push( $(this).data("value") );
            }
          } else {
            data[$(this).attr('name')] = $(this).val();
          }
  ...
  $.fn.refreshFilter = function(options) {
